...
scene.fog = new THREE.FogExp2(0x224466, 0.035);
...
function spawnSpiritParticles(pos, type='yellow'){
  // crea un pequeño conjunto de partículas coloreadas
  const COUNT = 180;
  ...
  for (let i=0;i<COUNT;i++){
    ...
    colors[i3+0]=baseColor.r; colors[i3+1]=baseColor.g; colors[i3+2]=baseColor.b;
...
function addSpirit(x, z, type = 'yellow') {
  const y = getTerrainHeight(x, z) + 0.8 + Math.random() * 1.2; // distintos altos
  ...
  // pequeña luz interior para brillo
  const glow = new THREE.PointLight(color, 0.9, 6, 2.0);
  glow.position.set(0, 0, 0);
  orb.add(glow);

  // levitación simple (animación por usuario .userData)
...
for (let i=0;i<SPIRIT_COUNT;i++){
  ...
  const radius = 6 + Math.random() * (PUMPKIN_AREA || 80);
  const x = Math.cos(angle) * radius;
  const z = Math.sin(angle) * radius;
  const type = Math.random() < 0.72 ? 'yellow' : 'blue'; // ~72% amarillos
  addSpirit(x,z,type);
}
...
// Quitar cualquier rayo / pointer visual XR
...
for (const s of spiritColliders){
  ...
  if (dist < minD){
    ...
    if (spirit && !spirit.userData.collected){
      ...
      // efecto visual y partículas (usa color según tipo)
      spawnSpiritParticles(spirit.position.clone(), spirit.userData.type);

      // sonido ping (reutiliza chimeBuffer)
      if (chimeBuffer) { const sfx = new THREE.Audio(listener); sfx.setBuffer(chimeBuffer); sfx.setLoop(false); sfx.setVolume(0.85); sfx.play(); }

      // elimina la esfera del mundo
      scene.remove(spirit);
      // opcional: liberar memoria más tarde si quieres
    }
  }
}
...
function checkWin() {
  ...
  if (score >= GOAL_SCORE) {
    ...
    document.getElementById("winMessage").style.display = "grid";

    // Opcional: congelar movimiento
